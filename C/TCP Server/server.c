...
#define STR_SIZE 64
#define MAX_PORT 65536 // max port size (2^16)
...
void handle_request(int fd, char **argv)
{
  ...
  if (argv == NULL || argv[0] == NULL || argv[1] == NULL || argv[2] == NULL || strlen(argv[0]) <= 0 || strlen(argv[1]) <= 0 || strlen(argv[2]) <= 0)
  {
